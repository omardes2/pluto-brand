...
> **حالة التنفيذ:** ✅ مُنفَّذ ومُختبَر: الفروع والمستودعات والمواقع (2.1)؛ الكتالوج المرجعي (2.2)؛ المنتجات ووسائطها (2.3)؛ **المخزون (2.4)** — `/api/v1/inventory/*` (stocks / movements / ledger / receive / issue / transfer / reservations / adjustments) بمحرّك WAC ومنع السالب داخل معاملات، وحجب حقول التكلفة بـ `pricing.view_cost` (ADR-005/007/008/013/024)؛ **المشتريات (2.5)** — `/api/v1/purchasing/*` (`suppliers` CRUD؛ `orders` CRUD + submit/approve/cancel/close؛ `receipts` index/show/store/post؛ `returns` index/show/store/approve/post) مع مرور الاستلام/المرتجع حصريًا عبر محرّك المخزون وتوزيع التكلفة المحمّلة (ADR-025)؛ **المبيعات (2.6)** — `/api/v1/sales/orders` (CRUD + confirm/reserve/prepare/ready/ship/deliver/cancel) بآلة حالات ADR-010، والحجز/الاستهلاك/التحرير حصريًا عبر محرّك المخزون بتوقيت ADR-009، وتجميد سعر البند (ADR-026)؛ **الشحن (2.7)** — `/api/v1/geo/*` (قراءة الجغرافيا) و`/api/v1/shipping/shipments` (CRUD + dispatch/out-for-delivery/delay/customer-unavailable/deliver/fail/override-cost) بآلة حالات تُكمل جزء التوصيل وتُزامن الطلب، وتعيين مزوّدين متعدد، ولقطة تكلفة، وطبقة تكامل توصيل بعقود + Null Drivers (ADR-027)؛ **المدفوعات (2.8)** — `/api/v1/payments/*` (methods + CRUD + capture/refund/callback) بطبقة تكامل مزوّدين (عقد موحّد `PaymentProviderInterface` + مدير مزوّدين، COD/تحويل فعّالان)، وبنية معاملات عامة، وحالة دفع الطلب المشتقّة (ADR-028)؛ **المحاسبة (2.9)** — `/api/v1/accounting/*` (accounts + balance، journal-entries CRUD + post/reverse، reports/trial-balance) بمحرّك قيد مزدوج، قيود ودفتر غير قابلة للتعديل (عكس لا حذف)، أرصدة مشتقّة من الدفتر، وعزل عبر AccountingService والأحداث (ADR-029)؛ **CRM/العملاء (2.10)** — `/api/v1/crm/customers` (CRUD + notes/block/unblock/merge/duplicates) بتعدد هواتف/عناوين/جهات اتصال/ملاحظات، كشف تكرار بالهاتف، حظر/دمج، لقطة عميل ثابتة على الطلبات، وطبقة مراسلة provider-agnostic (ADR-030)؛ **سلة المتجر (3.1)** — `/api/v1/store/cart` (GET؛ items POST؛ items/{variant} PUT/PATCH+DELETE؛ cart DELETE) بوحدة `Store` وكل المنطق في `CartService`، سلة self-scoped بالمستخدم، تسعير لقطة من الكتالوج (عرض ترويجي وإلا التجزئة)، فحص إتاحة Σ(on_hand − reserved)، ولا حجز مخزون في السلة (يتم عند تأكيد الطلب — ADR-009/031)؛ **إتمام الشراء متعدّد الخطوات (3.2)** — هوية مزدوجة (مستخدم مُصادَق أو ضيف برمز `X-Cart-Token` عبر طبقة `store.identity`): `POST /store/checkout` (بدء جلسة) · `GET/PUT/PATCH /store/checkout/{session}` (تحديث لقطة الشحن/الدفع) · `POST /store/checkout/{session}/place` (الإتمام) — يحوّل السلة إلى طلب عبر الخدمات القائمة بمعاملة الإنشاء الذرّية المحفوظة (إنشاء + تأكيد + حجز مخزون + بدء دفع COD + تعليم السلة محوَّلة)، ملكية الجلسة مفحوصة، ويُصدِر أحداث Checkout المستقرّة (ADR-032/033)؛ **واجهة المتجر (3.3)** — مسارات ويب SSR (`/`, `/shop`, `/search`, `/categories`, `/brands`, `/c/{slug}`, `/b/{slug}`, `/p/{slug}`, `/cart`, `/checkout`, `/lang/{locale}`) عربية RTL + إنجليزية، تقرأ عبر `StorefrontService` (يُعيد استخدام تسعير/توافر `CartService`)، والسلة/الإتمام من العميل عبر واجهات 3.1/3.2 بهوية مزدوجة؛ SEO (slug/meta/OG/canonical/JSON-LD) وجاهزية توصيات/عروض/تحليلات كنقاط امتداد (ADR-034)؛ **تجربة حساب العميل (3.4)** — مسارات ويب بجلسة (`/account/*`): تسجيل/دخول/خروج مع **دمج سلة الضيف**، لوحة، ملف/كلمة مرور، «طلباتي» + تتبّع (+ `orders/{order}/status` JSON) + إعادة الطلب، عناوين (CRUD + افتراضي)، مفضّلة، تفضيلات، ومركز إشعارات — تُعيد استخدام CustomerService/CartService/OrderService، صفحات `noindex`، وحقول جاهزية (ميلاد/تفضيلات/قنوات) بلا منطق نمو (ADR-035)؛ **المصادقة والهوية (3.5)** — دخول/تسجيل (بريد أو هاتف) مع قبول الشروط، استرجاع كلمة المرور (`/account/forgot-password`, `/account/reset-password`)، **دخول اجتماعي** (`/auth/{provider}/redirect|callback` لـ Google/Facebook عبر Socialite خلف عقد قابل للتبديل، الأسرار في `.env`)، هويّات `social_identities` بلا تكرار حسابات، إكمال الملف قبل الشراء (`/account/complete-profile`)، إدارة المزوّدين (`/account/providers` ربط/فكّ)، ودمج ضيف موسّع (سلة + طلبات مؤهّلة) — كلّه عبر الخدمات القائمة (ADR-036)؛ **البيع المُساعد (4.1)** — `POST /api/v1/sales/assisted-orders` (`AssistedOrderService` يُعيد استخدام `OrderService`/`CustomerService`) لإنشاء طلب من قناة (whatsapp/instagram/…) ببحث/إنشاء عميل بالهاتف وتعديل سعر يدوي مضبوط بالموافقة ولقطات كاملة + دفتر `order_price_changes` غير قابل للتعديل، و`GET/POST .../price-changes[/{id}/approve|reject]` (صلاحية `sales.orders.override_price`) — ADR-037؛ **دفاتر العمولة/الأرباح (4.2)** — `GET /api/v1/commissions` (فلترة بالحالة/المستفيد، تحكّم `view_own`/`view_team`)، `GET .../statement`، `POST .../approve` (صلاحية `commissions.approve`)، `POST .../payout` (صلاحية `commissions.payout`)، و`apiResource commissions/rules` (صلاحية `commissions.rules.manage`) — استحقاق تلقائي عند `OrderDelivered` بعمولة افتراضية 1% قابلة للضبط، دفاتر غير قابلة للتعديل (حركات موقّعة + سجل انتقالات + مدفوعات)، أسبقية قواعد حتمية، منع اعتماد/دفع مزدوج، والأهلية مؤجّلة للتسوية (4.6) — ADR-037؛ **محرّك حالة التوصيل القانوني (4.3)** — `GET /api/v1/shipping/shipments/{shipment}/delivery` (الحالة القانونية + السجلّان المنفصلان، صلاحية `shipping.delivery.view`)، `POST .../delivery/transition` (انتقال يدوي، `shipping.delivery.manage`)، `POST .../delivery/provider-status` (استيعاب حالة مزوّد خام idempotent، `shipping.delivery.sync`)، `POST .../delivery/close` (الإغلاق المالي — يُفعّل التسوية واستحقاق العمولات `eligible` بلا دفع تلقائي، `shipping.delivery.close`)، و`GET .../delivery/hold-reasons` (تقرير أسباب التعليق) — حالة قانونية منفصلة تمامًا عن حالة المزوّد الخام، التعيين في الـDriver (`OpostDeliveryProvider`)، سجلّان append-only، وCLOSE نقطة الاكتمال المالي الوحيدة — ADR-038؛ **بنية عمليات التوصيل (بقية 4.3، ADR-039)** — webhook عامّ `POST /api/v1/webhooks/delivery/{provider}` (تحقّق توقيع في الـDriver + idempotency + تسجيل كامل)؛ استثناءات `GET/POST /api/v1/shipping/shipments/{shipment}/exceptions` و`POST .../delivery/exceptions/{exception}/{note|assign|escalate|resolve|reopen}` (SLA/تصعيد، صلاحية `shipping.delivery.manage`)؛ رسوم `GET /api/v1/shipping/shipments/{shipment}/fees` + `POST` (مكوّنات مستقلّة عن المزوّد، صلاحية `shipping.delivery.fees`)؛ و`GET .../delivery/timeline` (خطّ زمني موحّد) — كلّها عبر تجريد المزوّد (`DeliveryProviderManager`، دعم عدّة مزوّدين)، مع أمري `delivery:sync`/`delivery:escalate-exceptions` المجدولين — ADR-039؛ **المرتجعات والاستبدال RMA (4.4)** — `GET/POST /api/v1/rma` (قائمة/إنشاء)، `GET /api/v1/rma/{return}` (تفاصيل + خطّ زمني)، `POST .../photo` (صورة دليل)، و`POST .../{approve|reject|cancel|receive|inspect|complete}` بسير موافقات مبيعات→مشرف→مستودع→قرار نهائي (صلاحيات `returns.{create,approve,receive,inspect,finalize}`) — يُعيد استخدام المخزون/المدفوعات/العمولة/الطلبات/الشحن دون تكرار، الفحص يوجّه المخزون (return_in/damage_out مسجَّلة)، والاستبدال يُنشئ شحنة مرتبطة لا يعدّل الأصل — ADR-040؛ **التسويات المالية (4.6)** — `GET/POST /api/v1/settlements` (قائمة/إنشاء)، `GET /api/v1/settlements/{settlement}`، `POST .../shipments` (إضافة شحنات مُغلقة)، `POST .../lines` (سطر يدوي/خصم)، `POST .../reconcile` (مطابقة وكشف تباين، صلاحية `settlements.reconcile`)، `POST .../post` (ترحيل محاسبي مزدوج عبر `AccountingService` + تأكيد الاستحقاق، صلاحية `settlements.post`)، و`POST .../{close|cancel}` — يُعيد استخدام المحاسبة والعمولات دون تكرار، حساب COD clearing `1050` مُضاف — ADR-041. **المرحلة 4.5 (مطالبات) مُستبعَدة**؛ **التقارير والتحليلات (4.7)** — لوحات/تقارير **ويب فقط** (للقراءة) تحت `/admin/reports/*` (`can:reports.view`): تنفيذية/مبيعات/طلبات/توصيل/مالية + أداء موظفين/مسوّقين/منتجات + إحصاءات عملاء/شركات توصيل/مرتجعات، بنطاقات يومي/أسبوعي/شهري/مخصّص، تصدير CSV (Excel) وطباعة (PDF) — تجميعات قراءة فوق الجداول القائمة بلا تكرار منطق (ADR-042). **بذلك تكتمل المرحلة 4.** **إدارة المستودع والمخزون (المرحلة 5، ADR-043)** — تعزيزات فوق محرّك المخزون القائم: `GET /api/v1/inventory/barcode?code=` (بحث باركود)، `GET .../low-stock` (تنبيهات نقص، `inventory.alerts.view`)، `GET .../warehouse-dashboard` (مؤشّرات)، و**الجرد الفعلي** `GET/POST /api/v1/inventory/counts` + `GET .../counts/{count}` + `POST .../counts/{count}/{record|review|complete|cancel}` (`inventory.counts.{view,manage}`) — الإكمال يطبّق الفروق حصريًا عبر `InventoryService` (حركة/دفتر/تدقيق تلقائي، لا مساس مباشر). لوحة/جرد/دفعات في `/admin/inventory/*`.
