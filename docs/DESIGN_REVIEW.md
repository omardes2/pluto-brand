...
**المشروع:** Pluto Brand (Pluto Brand)
**المرحلة:** تحليل الأعمال وتصميم Phase 2 (قبل التنفيذ)
**التاريخ:** 2026-07-11
**الحالة:** ✅ مُعتمد ومُجمَّد (Frozen) — 2026-07-11. كل الأسئلة محسومة، وتنفيذ Phase 2 بدأ.
...
أُنجزت مرحلة **تحليل الأعمال والتصميم** بالكامل قبل كتابة أي كود لـ Phase 2. تُرجمت رؤية "Pluto Brand" (منصة ERP + CRM + متجر + مخزون + مشتريات + محاسبة + عمولات + تواصل متعدد القنوات) إلى:
...
## 7. الأسئلة المحسومة (Resolved — مُجمَّدة 2026-07-11)

كل الأسئلة المفتوحة **حُسمت واعتُمدت** بموافقة المالك، وثُبّتت في `DECISIONS.md` (قسم "قرارات محسومة"):

| # | القرار | القيمة النهائية | المرجع |
|:-:|--------|-----------------|--------|
...
| 5 | العملة في الدفاتر | أحادية العملة (`SAR`) | ADR-001 |
| 6 | منع المخزون السالب | ممنوع افتراضيًا (قابل للتفعيل لكل مستودع) | ADR-007a |
| 7 | حدّ اعتماد الخصم | 10% افتراضيًا (قابل للإعداد) | ADR-006a |
| 8 | نافذة الإرجاع بعد التسليم | 14 يومًا (قابلة للإعداد) | ADR-011 |
| 9 | `reports.view_financial` مقابل `pricing.view_cost` | صلاحيتان منفصلتان | ADR-013/021 |

> ✅ **لا أسئلة مفتوحة متبقّية.** حزمة التحليل والتصميم **مُجمَّدة**، والتنفيذ (Phase 2) بدأ وفق هذه القرارات.
...
حزمة التحليل والتصميم **مكتملة ومتّسقة ومُجمَّدة ومدفوعة**، وكل الأسئلة محسومة (القسم 7).

**بدأ تنفيذ Phase 2** وفق التسلسل المعتمد (feature-by-feature: تنفيذ كامل → اختبار → التزام → دفع → مراجعة قبل الانتقال). نقطة البدء: **Phase 2.1 — الفروع، المستودعات، مواقع التخزين**.
