...
**Pluto Brand (بلوتو براند)** — منصة أعمال متكاملة تجمع **ERP + CRM + متجر إلكتروني** في نظام واحد. مشروع مستقل بالكامل (قاعدة بيانات ومفتاح وبادئات كاش/Redis خاصة به). راجع `README.md` و`REQUIREMENTS.md` للنطاق الكامل.
...
- **الفرع:** طوّر على `main` (الفرع الرئيسي الوحيد لهذا المشروع المستقل).
- **الالتزامات (Commits):** رسائل واضحة ووصفية. التزام واحد لكل وحدة عمل منطقية.
- **الدفع:** `git push -u origin main` بعد كل وحدة عمل.
