...
<body class="min-h-screen flex flex-col bg-gray-50 text-gray-800 antialiased">
    ...
    @if ($freeShip)
        <div class="bg-gray-900 text-white text-center text-sm py-2 px-4">
    ...
    <header class="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <form action="{{ route('storefront.search') }}" method="GET" role="search"
                    ...
                    <div class="relative">
                        <input id="sf-search" type="search" name="q" value="{{ request('q') }}"
                               ...
                               class="w-full rounded-full border-gray-300 ps-4 pe-10 py-2 text-sm focus:border-gray-900 focus:ring-gray-900">
                        <button type="submit" aria-label="{{ __('storefront.search') }}"
                                class="absolute inset-y-0 end-0 flex items-center pe-3 text-gray-400 hover:text-gray-900">
                    ...
                    <a href="{{ route('storefront.cart') }}" class="relative p-2 rounded-md text-gray-700 hover:bg-gray-100"
                       aria-label="{{ __('storefront.cart') }}">
                        ...
                        <span x-data x-show="$store.cart.count > 0" x-text="$store.cart.count" x-cloak
                              class="absolute -top-0.5 -end-0.5 min-w-5 h-5 px-1 inline-flex items-center justify-center rounded-full bg-gray-900 text-white text-xs font-bold"></span>
                    </a>
                </div>
            </div>
            ...
            <nav class="flex items-center gap-4 h-11 text-sm overflow-x-auto" aria-label="{{ __('storefront.menu') }}">
                <a href="{{ route('storefront.shop') }}" class="text-gray-600 hover:text-gray-900 whitespace-nowrap">{{ __('storefront.shop') }}</a>
                <a href="{{ route('storefront.categories') }}" class="text-gray-600 hover:text-gray-900 whitespace-nowrap">{{ __('storefront.categories') }}</a>
                <a href="{{ route('storefront.brands') }}" class="text-gray-600 hover:text-gray-900 whitespace-nowrap">{{ __('storefront.brands') }}</a>
            </nav>
        </div>
    </header>
    ...
    <footer class="bg-white border-t border-gray-200 mt-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 text-sm text-gray-500 flex flex-col sm:flex-row items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('storefront.categories') }}" class="hover:text-gray-900">{{ __('storefront.categories') }}</a>
                <a href="{{ route('storefront.brands') }}" class="hover:text-gray-900">{{ __('storefront.brands') }}</a>
            </div>
        </div>
    </footer>
