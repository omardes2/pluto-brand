    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            @forelse ($addresses as $address)
                <div class="bg-white rounded-xl border border-gray-200 p-4" x-data="{ editing: false }">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                ...
                                @if ($address->is_default)
                                    <span class="text-xs bg-gray-100 text-black px-2 py-0.5 rounded-full">{{ __('account.default') }}</span>
                                @endif
                            </div>
                        ...
                        <button type="button" @click="editing = !editing" class="text-gray-400 hover:text-gray-900 text-sm shrink-0">{{ __('account.edit_address') }}</button>
                    </div>
                    ...
                    <div class="flex items-center gap-3 mt-3">
                        @unless ($address->is_default)
                            <form method="POST" action="{{ route('account.addresses.default', $address) }}">
                                @csrf
                                <button type="submit" class="text-xs text-gray-900 hover:underline">{{ __('account.set_default') }}</button>
                            </form>
                        @endunless
                    ...
                    <form x-show="editing" x-cloak method="POST" action="{{ route('account.addresses.update', $address) }}" class="mt-3 space-y-2 border-t border-gray-100 pt-3">
                        ...
                        <button type="submit" class="rounded-lg bg-gray-900 hover:bg-black text-white text-sm px-4 py-2">{{ __('account.save') }}</button>
                    </form>
                </div>
            @empty
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 h-fit">
            ...
            <form method="POST" action="{{ route('account.addresses.store') }}" class="space-y-3">
                @csrf
                <input name="label" value="{{ old('label') }}" placeholder="{{ __('account.label') }}" class="w-full rounded-lg border-gray-300 text-sm focus:border-gray-900 focus:ring-gray-900">
                <input name="recipient_name" value="{{ old('recipient_name') }}" placeholder="{{ __('account.recipient_name') }}" class="w-full rounded-lg border-gray-300 text-sm focus:border-gray-900 focus:ring-gray-900">
                <input name="phone" value="{{ old('phone') }}" placeholder="{{ __('account.phone') }}" inputmode="tel" class="w-full rounded-lg border-gray-300 text-sm focus:border-gray-900 focus:ring-gray-900">
                <textarea name="address_line" rows="3" required placeholder="{{ __('account.address_line') }}" class="w-full rounded-lg border-gray-300 text-sm focus:border-gray-900 focus:ring-gray-900">{{ old('address_line') }}</textarea>
                <label class="flex items-center gap-2 text-sm text-gray-600">
                    <input type="checkbox" name="is_default" value="1" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                    {{ __('account.set_default') }}
                </label>
                <button type="submit" class="w-full rounded-lg bg-gray-900 hover:bg-black text-white text-sm font-medium py-2.5">{{ __('account.save') }}</button>
            </form>
        </div>
    </div>
