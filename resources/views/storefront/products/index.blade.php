    ...
    <div class="flex items-center justify-between gap-3 mb-4">
        ...
        <form action="{{ route('storefront.shop') }}" method="GET" class="shrink-0">
            ...
            <select id="sort" name="sort" onchange="this.form.submit()"
                    class="rounded-lg border-gray-300 text-sm py-2 focus:border-gray-900 focus:ring-gray-900">
        ...
        <aside class="lg:col-span-1" x-data="{ open: false }">
            ...
            <form action="{{ route('storefront.shop') }}" method="GET"
                ...
                <div>
                    ...
                    <select id="f-category" name="category" class="w-full rounded-lg border-gray-300 text-sm focus:border-gray-900 focus:ring-gray-900">
                ...
                <div>
                    ...
                    <select id="f-brand" name="brand" class="w-full rounded-lg border-gray-300 text-sm focus:border-gray-900 focus:ring-gray-900">
                        <option value="">{{ __('storefront.all') }}</option>
                        @foreach ($brands as $b)
                ...
                <div class="flex items-center gap-2 pt-1">
                    <button type="submit" class="flex-1 rounded-lg bg-gray-900 hover:bg-black text-white text-sm font-medium py-2">{{ __('storefront.apply') }}</button>
        ...
        <div class="lg:col-span-3">
            @if ($products->isEmpty())
                <div class="bg-white rounded-xl border border-dashed border-gray-300 p-10 text-center">
                    ...
                    <a href="{{ route('storefront.shop') }}" class="inline-block mt-4 text-sm text-gray-900 hover:underline">{{ __('storefront.back_to_shop') }}</a>
                </div>
            @else
