    ...
    <div x-data>
        ...
        <template x-if="$store.cart.error && !$store.cart.loading">
            <div class="bg-white rounded-xl border border-rose-200 p-6 text-center">
                ...
                <button type="button" @click="$store.cart.refresh()" class="mt-3 text-sm text-gray-900 hover:underline">{{ __('storefront.retry') }}</button>
            </div>
        </template>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-xl border border-gray-200 p-5 sticky top-24">
                        ...
                        <a href="{{ route('storefront.checkout') }}" class="block text-center rounded-lg bg-gray-900 hover:bg-black text-white font-semibold py-3">{{ __('storefront.proceed_to_checkout') }}</a>
                        <a href="{{ route('storefront.shop') }}" class="block text-center mt-2 text-sm text-gray-500 hover:text-gray-900">{{ __('storefront.continue_shopping') }}</a>
                    </div>
                </div>
            </div>
        </template>
